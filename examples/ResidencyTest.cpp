...
// however the portion of textures is shifted such that some of the textures used in the beginning of the last frame are no longer used,
// and some new textures will be used at the end of the next frame. Because Dawn's residency manager is implemented as an LRU,
// this forces paging because the textures in the next frame should not be in device memory. What
// this workload tests, is that with a changing memory landscape - the most recently used resources
// remain in local memory.
...
#define TEXTURES_IN_RESOURCE_POOL 5000
